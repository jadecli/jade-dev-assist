...
test('1. updateTaskStatus is exported as a function', () => {
    assert(typeof updateTaskStatus === 'function', 'updateTaskStatus should be a function');
});

test('2. getTaskStatus is exported as a function', () => {
    assert(typeof getTaskStatus === 'function', 'getTaskStatus should be a function');
});

test('3. watchWorkerCompletion is exported as a function', () => {
    assert(typeof watchWorkerCompletion === 'function', 'watchWorkerCompletion should be a function');
});
...
test('5. updateTaskStatus records history entry', () => {
    ...
    try {
        ...
        assert(entry.from_status === 'pending', `Expected from_status "pending", got "${entry.from_status}"`);
        assert(entry.to_status === 'in_progress', `Expected to_status "in_progress", got "${entry.to_status}"`);
        assert(typeof entry.timestamp === 'string', 'History entry should have a timestamp');
    } finally {
        env.cleanup();
    }
});
...
test('7. updateTaskStatus updates the updated_at timestamp', () => {
    ...
    try {
        ...
        const data = JSON.parse(content);
        const task = data.tasks.find(t => t.id === 'test-project/test-task');
        assert(typeof task.updated_at === 'string', 'Task should have updated_at timestamp');
        assert(!isNaN(Date.parse(task.updated_at)), 'updated_at should be valid ISO date');
    } finally {
        env.cleanup();
    }
});
...
queueAsync('12. watchWorkerCompletion updates status to completed on exit 0', async () => {
    const env = createTestProject();
    try {
        ...
        const result = await watchWorkerCompletion('test-project/test-task', child, {
            projectsRoot: env.tmpdir
        });
...
queueAsync('13. watchWorkerCompletion records history entry on success', async () => {
    const env = createTestProject();
    try {
        updateTaskStatus('test-project/test-task', 'in_progress', {
            projectsRoot: env.tmpdir
        ...
        assert(typeof completedEntry.timestamp === 'string', 'Entry should have timestamp');
    } finally {
        env.cleanup();
    }
});

queueAsync('14. watchWorkerCompletion returns result with exitCode 0', async () => {
    const env = createTestProject();
    try {
        updateTaskStatus('test-project/test-task', 'in_progress', {
            projectsRoot: env.tmpdir
...
queueAsync('15. watchWorkerCompletion updates status to failed on non-zero exit', async () => {
    const env = createTestProject();
    try {
        updateTaskStatus('test-project/test-task', 'in_progress', {
            projectsRoot: env.tmpdir
...
queueAsync('16. watchWorkerCompletion records history with error details on failure', async () => {
    const env = createTestProject();
    try {
        updateTaskStatus('test-project/test-task', 'in_progress', {
            projectsRoot: env.tmpdir
        ...
        assert(typeof failedEntry.timestamp === 'string', 'Entry should have timestamp');
        assert(
            typeof failedEntry.agent_summary === 'string' && failedEntry.agent_summary.length > 0,
...
queueAsync('17. watchWorkerCompletion returns result with non-zero exitCode', async () => {
    const env = createTestProject();
    try {
        updateTaskStatus('test-project/test-task', 'in_progress', {
            projectsRoot: env.tmpdir

...
function createTestEnv(options) {
  ...
  if (options.taskFiles) {
    for (const [projectPath, fixtureName] of Object.entries(
      options.taskFiles
    )) {
...
test('1. loadRegistry reads valid projects.json', () => {
  ...
  try {
    ...
    assert(
      ...
      'Expected projects to be an array'
    );
    assert(registry.projects.length === 1, 'Expected 1 project');
    assert(
      registry.projects[0].name === 'jade-cli',
      'Expected project name jade-cli'
    );
  } finally {
    env.cleanup();
  }
  ...
  try {
    ...
    try {
      ...
      assert(
        ...
        `Expected SyntaxError, got ${err.constructor.name}`
      );
    }
...
test('4. scanTasks returns merged tasks from multiple projects', () => {
  ...
  try {
    ...
    assert(
      ...
      'Expected result to have tasks property'
    );
    assert(Array.isArray(result.tasks), 'Expected tasks to be an array');
    assert(
      result.tasks.length === 3,
      `Expected 3 tasks (2 from jade-cli + 1 from jade-index), got ${result.tasks.length}`
    );

    const ids = result.tasks.map((t) => t.id);
    assert(
      ids.includes('jade-cli/fix-node-build'),
      'Expected jade-cli/fix-node-build task'
    );
    assert(
      ids.includes('jade-cli/add-task-create'),
      'Expected jade-cli/add-task-create task'
    );
    assert(
      ids.includes('jade-index/add-semantic-search'),
      'Expected jade-index/add-semantic-search task'
    );
  } finally {
    env.cleanup();
  }
...
test('5. scanTasks tags each task with _projectName', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected 2 tasks, got ${result.tasks.length}`
    );
    for (const task of result.tasks) {
      assert(
        task._projectName === 'jade-cli',
        `Expected _projectName 'jade-cli', got '${task._projectName}'`
      );
    }
  } finally {
    env.cleanup();
...
test('6. scanTasks tags each task with _project (full registry entry)', () => {
  ...
  try {
    ...
    assert(
      task._project.name === 'jade-cli',
      `Expected _project.name 'jade-cli', got '${task._project.name}'`
    );
    assert(
      task._project.status === 'near-buildable',
      `Expected _project.status 'near-buildable', got '${task._project.status}'`
    );
    assert(
      task._project.language === 'typescript',
      `Expected _project.language 'typescript', got '${task._project.language}'`
    );
    assert(
      task._project.test_command === 'npx vitest run',
      `Expected _project.test_command 'npx vitest run'`
    );
  } finally {
    env.cleanup();
  }
...
test('7. scanTasks tags each task with _milestone from file-level milestone', () => {
  ...
  try {
    ...
    assert(
      task._milestone.name === 'Core Commands',
      `Expected _milestone.name 'Core Commands', got '${task._milestone.name}'`
    );
    assert(
      task._milestone.target_date === '2026-03-15',
      `Expected _milestone.target_date '2026-03-15'`
    );
  } finally {
    env.cleanup();
  }
...
test('8. scanTasks handles missing tasks.json gracefully (returns 0 tasks for that project)', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected 0 tasks, got ${result.tasks.length}`
    );
  } finally {
    fs.rmSync(tmpdir, { recursive: true, force: true });
  }
...
test('9. scanTasks handles malformed JSON gracefully (warns, continues with other projects)', () => {
  ...
  try {
    ...
    assert(
      logEntry.level === 'warn',
      `Expected level warn, got ${logEntry.level}`
    );
    assert(
      logEntry.module === 'scanner',
      `Expected module scanner, got ${logEntry.module}`
    );
    ...
    assert(
      ...
      `Expected 1 task from jade-index, got ${result.tasks.length}`
    );
    assert(
      result.tasks[0].id === 'jade-index/add-semantic-search',
      'Expected the jade-index task'
    );
  } finally {
    env.cleanup();
  }
...
test('13. scanTasks accepts pre-loaded registry object (options.registry instead of options.registryPath)', () => {
  ...
  try {
    ...
    assert(
      result.tasks[0]._projectName === 'jade-cli',
      'Expected _projectName to be jade-cli'
    );
  } finally {
    env.cleanup();
  }
...
test('14. scanTasks returns partial results with error list on malformed JSON', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected 1 error, got ${result.errors.length}`
    );
    assert(
      result.errors[0].type === 'parse_error',
      `Expected parse_error type, got ${result.errors[0].type}`
    );
    assert(
      result.errors[0].project === 'bad-project',
      'Expected bad-project in error'
    );
  } finally {
    env.cleanup();
  }
...
test('15. scanTasks strict mode throws on parse error', () => {
  ...
  try {
    ...
    assert(
      thrownError.code === 'SCANNER_STRICT_ERROR',
      `Expected SCANNER_STRICT_ERROR, got ${thrownError.code}`
    );
    assert(
      Array.isArray(thrownError.errors),
      'Expected errors attached to thrown error'
    );
...
test('16. scanTasks returns warnings for unknown fields', () => {
  ...
  try {
    ...
    assert(
      result.tasks.length === 1,
      `Expected 1 task, got ${result.tasks.length}`
    );
    ...
    assert(
      ...
      `Expected 2 warnings for unknown fields, got ${result.warnings.length}`
    );

    const warnMessages = result.warnings.map((w) => w.message);
    assert(
      warnMessages.some((m) => m.includes('unknown_field')),
      'Expected warning about unknown_field'
    );
    assert(
      warnMessages.some((m) => m.includes('another_unknown')),
      'Expected warning about another_unknown'
    );
  } finally {
    env.cleanup();
  }
...
test('17. scanTasks strict mode throws on unknown fields (warnings treated as errors)', () => {
  ...
  try {
    ...
    assert(
      Array.isArray(thrownError.warnings),
      'Expected warnings attached to thrown error'
    );
  } finally {
    env.cleanup();
  }
...
test('18. validateTask detects missing required fields', () => {
  ...
  assert(
    ...
    'Expected error about missing status'
  );
});
...
test('19. validateTask passes for valid task with required fields only', () => {
  ...
  assert(
    ...
    `Expected 0 errors, got ${result.errors.length}`
  );
});
...
test('20. validateTask warns on unknown fields', () => {
  ...
  assert(
    result.valid,
    'Expected valid result (warnings dont invalidate in non-strict mode)'
  );
  assert(
    result.warnings.length === 1,
    `Expected 1 warning, got ${result.warnings.length}`
  );
  assert(
    result.warnings[0].includes('mystery_field'),
    'Expected warning about mystery_field'
  );
});
...
test('21. applyTaskDefaults sets defaults for optional fields', () => {
  ...
  assert(
    task.complexity === 'M',
    `Expected default complexity 'M', got '${task.complexity}'`
  );
...
test('22. applyTaskDefaults does not overwrite existing values', () => {
  ...
  assert(
    task.complexity === 'L',
    `Expected complexity 'L', got '${task.complexity}'`
  );
...
test('23. scanTasks skips tasks with missing required fields', () => {
  ...
  try {
    ...
    assert(
      result.tasks.length === 1,
      `Expected 1 valid task, got ${result.tasks.length}`
    );
    ...
    assert(
      result.errors.length === 2,
      `Expected 2 validation errors, got ${result.errors.length}`
    );
  } finally {
    env.cleanup();
  }
...
test('24. scanTasks applies defaults to tasks with optional fields missing', () => {
  ...
  try {
    ...
    assert(
      ...
      'Expected blocked_by to be an array'
    );
    assert(Array.isArray(task.unlocks), 'Expected unlocks to be an array');
  } finally {
    env.cleanup();
...
test('25. REQUIRED_TASK_FIELDS contains id, title, status', () => {
  assert(
    REQUIRED_TASK_FIELDS.includes('id'),
    'Expected id in REQUIRED_TASK_FIELDS'
  );
  assert(
    REQUIRED_TASK_FIELDS.includes('title'),
    'Expected title in REQUIRED_TASK_FIELDS'
  );
  assert(
    REQUIRED_TASK_FIELDS.includes('status'),
    'Expected status in REQUIRED_TASK_FIELDS'
  );
  assert(
    REQUIRED_TASK_FIELDS.length === 3,
    `Expected 3 required fields, got ${REQUIRED_TASK_FIELDS.length}`
  );
});

test('26. OPTIONAL_TASK_FIELDS has correct defaults', () => {
  assert(
    OPTIONAL_TASK_FIELDS.complexity === 'M',
    'Expected complexity default to be M'
  );
  assert(
    Array.isArray(OPTIONAL_TASK_FIELDS.blocked_by),
    'Expected blocked_by default to be array'
  );
  assert(
    OPTIONAL_TASK_FIELDS.blocked_by.length === 0,
    'Expected blocked_by default to be empty'
  );
  assert(
    Array.isArray(OPTIONAL_TASK_FIELDS.unlocks),
    'Expected unlocks default to be array'
  );
  assert(
    OPTIONAL_TASK_FIELDS.unlocks.length === 0,
    'Expected unlocks default to be empty'
  );
});

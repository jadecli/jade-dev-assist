...
test('1. Renders table with box-drawing chars', () => {
  const tasks = [
    ...
    makeRankedTask({
      _projectName: 'jade-index',
      title: 'Add semantic search',
      _score: 65.0,
      complexity: 'M',
    }),
    makeRankedTask({
      _projectName: 'jade-dev-assist',
...
test('3. Truncates project names >16 chars', () => {
  ...
  const tasks = [makeRankedTask({ _projectName: longName })];
  const mockStream = createMockStream();
  presentTasks(tasks, { output: mockStream });
  const output = mockStream.getOutput();
  ...
  assert(
    ...
    `Expected truncated name "${truncated}" in output`
  );
});
...
test('4. Truncates task titles >40 chars', () => {
  const longTitle =
    'A very long task title that exceeds forty characters limit'; // 58 chars
  const tasks = [makeRankedTask({ title: longTitle })];
  const mockStream = createMockStream();
  presentTasks(tasks, { output: mockStream });
  const output = mockStream.getOutput();
  ...
  assert(
    output.includes(truncated),
    `Expected truncated title "${truncated}" in output`
  );
});
...
test('8. Handles empty task list', () => {
  ...
  assert(
    ...
    'Expected "(no tasks)" message for empty list'
  );
});
...
test('9. Writes to custom output stream', () => {
  ...
  assert(
    !stdoutCalled,
    'process.stdout should not have been written to when custom output is provided'
  );
});
...
test('10. Returns the formatted string', () => {
  ...
  assert(
    ...
    `Expected return type string, got ${typeof result}`
  );
  assert(result.length > 0, 'Return value should not be empty');
  assert(
    result.includes('Return value test'),
    'Return value should contain the task title'
  );
  assert(
    result.includes('\u2502'),
    'Return value should contain box-drawing characters'
  );
  assert(
    result === mockStream.getOutput(),
    'Return value should match stream output'
  );
});

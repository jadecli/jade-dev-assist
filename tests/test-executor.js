...
function createMockSpawn(exitCode) {
    ...
    function mockSpawn(command, args, options) {
        ...
        process.nextTick(() => {
            if (exitCode === 0) {
                child.stdout.emit('data', Buffer.from('Worker output: task completed successfully\n'));
            } else {
                child.stderr.emit('data', Buffer.from('Worker error: something went wrong\n'));
            }
...
test('1. executeWorker is exported as a function', () => {
    assert(typeof executeWorker === 'function', 'executeWorker should be a function');
});
...
queueAsync('2. executeWorker returns a promise', async () => {
    ...
    try {
        ...
        assert(typeof result.then === 'function', 'Result should be a promise (thenable)');
...
queueAsync('3. executeWorker resolves with output on exit code 0', async () => {
    ...
    try {
        ...
        assert(result != null, 'Result should not be null');
        assert(typeof result.stdout === 'string', 'Result should have stdout string');
        assert(result.stdout.includes('task completed'), 'stdout should contain worker output');
...
queueAsync('4. executeWorker rejects or returns failure on non-zero exit', async () => {
    const env = createTestProject({ claudeMd: '# Test\n' });
    try {
...
queueAsync('5. spawns "claude" with --print and --dangerouslySkipPermissions flags', async () => {
    const env = createTestProject({ claudeMd: '# Test\n' });
    try {
        ...
        const call = calls[0];
        assert(call.command === 'claude', `Expected command "claude", got "${call.command}"`);
...
queueAsync('10. records history entry with timestamp on success', async () => {
    ...
    try {
        ...
        const completedEntry = task.history.find(h => h.to_status === 'completed');
...
queueAsync('15. emits stdout data events via onStdout callback', async () => {
    ...
    try {
        ...
        await executeWorker('test-project/test-task', {
            projectsRoot: env.tmpdir,
            _spawnFn: mockSpawn,
            onStdout: (data) => { stdoutChunks.push(data.toString()); }
        });
        assert(stdoutChunks.length > 0, 'onStdout should have been called at least once');
        assert(
    ...
    try {
        ...
        try {
            await executeWorker('test-project/test-task', {
                projectsRoot: env.tmpdir,
                _spawnFn: mockSpawn,
                onStderr: (data) => { stderrChunks.push(data.toString()); }
            });
        } catch (_err) {
            // Expected
        }
        assert(stderrChunks.length > 0, 'onStderr should have been called at least once');
        assert(
        ...
        tasks: [
            {
                ...
                unlocks: [],
                labels: ['feature'],
                feature: { description: 'Alpha feature', acceptance_criteria: ['alpha criterion'] },
                relevant_files: [],
                created_at: '2026-02-02T00:00:00Z'
            },
            {
                ...
                unlocks: [],
                labels: ['feature'],
                feature: { description: 'Beta feature', acceptance_criteria: ['beta criterion'] },
                relevant_files: [],
...
queueAsync('18. throws/rejects if taskId is not found in tasks.json', async () => {
    const env = createTestProject({ claudeMd: '# Test\n' });
    try {
        const { mockSpawn } = createMockSpawn(0);
        let threw = false;
        try {
            ...
            threw = true;
            assert(
                err.message.includes('not found') || err.message.includes('nonexistent'),
...
queueAsync('20. result includes execution timestamps', async () => {
    ...
    try {
        ...
        assert(typeof result.startedAt === 'string', 'Result should have startedAt timestamp');
        assert(typeof result.completedAt === 'string', 'Result should have completedAt timestamp');
        // Validate they are ISO date strings
        assert(!isNaN(Date.parse(result.startedAt)), 'startedAt should be a valid ISO date');
        assert(!isNaN(Date.parse(result.completedAt)), 'completedAt should be a valid ISO date');
    } finally {
        env.cleanup();
    }
});
...
test('21. buildTierConfig is exported as a function', () => {
    assert(typeof buildTierConfig === 'function', 'buildTierConfig should be a function');
});

test('22. getOllamaBaseUrl is exported as a function', () => {
    assert(typeof getOllamaBaseUrl === 'function', 'getOllamaBaseUrl should be a function');
});
...
test('24. opus tier returns base args with no --model flag', () => {
    ...
    assert(config.args.includes('--dangerouslySkipPermissions'), 'Should include --dangerouslySkipPermissions');
    assert(!config.args.includes('--model'), 'Should NOT include --model flag for opus tier');
...
test('28. default tier (undefined modelTier) behaves as opus', () => {
    ...
    assert(Array.isArray(config.args), 'config.args should be an array');
    assert(!config.args.includes('--model'), 'Should NOT include --model flag when modelTier is undefined');
    assert(config.env === undefined, 'env should be undefined when modelTier is undefined');
});

test('29. default tier (null modelTier) behaves as opus', () => {
    const config = buildTierConfig(null);
    assert(Array.isArray(config.args), 'config.args should be an array');
    assert(!config.args.includes('--model'), 'Should NOT include --model flag when modelTier is null');
    assert(config.env === undefined, 'env should be undefined when modelTier is null');
});
...
queueAsync('33. opus tier task spawns claude with no --model flag', async () => {
    ...
    try {
        ...
        const call = calls[0];
        assert(call.command === 'claude', `Expected command "claude", got "${call.command}"`);
        assert(!call.args.includes('--model'), 'Opus tier should NOT include --model flag');
        assert(call.args.includes('--print'), 'Should include --print');
        assert(call.args.includes('--dangerouslySkipPermissions'), 'Should include --dangerouslySkipPermissions');
    } finally {
        env.cleanup();
    }
});
...
queueAsync('35. local tier task spawns claude with --model qwen3-coder', async () => {
...
queueAsync('36. local tier task sets ANTHROPIC_AUTH_TOKEN and ANTHROPIC_BASE_URL env vars', async () => {
    ...
    try {
        ...
        const spawnOpts = calls[0].options;
        assert(spawnOpts.env != null, 'Local tier should set env on spawn options');
        assert(
...
queueAsync('37. default tier (no modelTier on task) behaves as opus', async () => {
    ...
    try {
        ...
        assert(calls.length === 1, `Expected 1 spawn call, got ${calls.length}`);
        const call = calls[0];
        assert(!call.args.includes('--model'), 'Default tier (no modelTier) should NOT include --model flag');
...
queueAsync('38. OLLAMA_BASE_URL config overrides default in local tier integration', async () => {
    const savedOllama = process.env.OLLAMA_BASE_URL;
...
queueAsync('39. local tier task still includes --print and --dangerouslySkipPermissions', async () => {
    const env = createTieredTestProject('local');
    try {
        const { mockSpawn, calls } = createMockSpawn(0);
        await executeWorker('test-project/tier-task', {
        ...
        const call = calls[0];
        assert(call.args.includes('--print'), 'Local tier should still include --print');
        assert(
...
queueAsync('40. local tier task still updates status to completed on success', async () => {
    const env = createTieredTestProject('local');
    try {
        const { mockSpawn } = createMockSpawn(0);
        await executeWorker('test-project/tier-task', {

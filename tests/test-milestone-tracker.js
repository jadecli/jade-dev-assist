  ...
  return createTestEnv({
    ...
    taskFiles: {
      ...
      'project-beta': {
        ...
        tasks: [
          {
            id: 'project-beta/task-1',
            title: 'Beta task 1',
            status: 'completed',
            complexity: 'S',
          },
          {
            id: 'project-beta/task-2',
            title: 'Beta task 2',
            status: 'pending',
            complexity: 'M',
          },
          {
            id: 'project-beta/task-3',
            title: 'Beta task 3',
            status: 'pending',
            complexity: 'L',
          },
...
test('1. getMilestoneProgress is exported as a function', () => {
  assert(
    typeof getMilestoneProgress === 'function',
    'getMilestoneProgress should be a function'
  );
});

test('2. getAllMilestonesProgress is exported as a function', () => {
  assert(
    typeof getAllMilestonesProgress === 'function',
    'getAllMilestonesProgress should be a function'
  );
});

test('3. renderMilestoneTable is exported as a function', () => {
  assert(
    typeof renderMilestoneTable === 'function',
    'renderMilestoneTable should be a function'
  );
});
...
test('4. getMilestoneProgress counts correctly for project-alpha', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected completed 2, got ${progress.completed}`
    );
    assert(
      progress.inProgress === 1,
      `Expected inProgress 1, got ${progress.inProgress}`
    );
    assert(
      progress.pending === 1,
      `Expected pending 1, got ${progress.pending}`
    );
  } finally {
    env.cleanup();
  }
...
test('5. getMilestoneProgress computes correct percentage', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected percentage 50, got ${progress.percentage}`
    );
  } finally {
    env.cleanup();
  }
...
test('6. getMilestoneProgress counts correctly for project-beta', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected completed 1, got ${progress.completed}`
    );
    assert(
      progress.inProgress === 0,
      `Expected inProgress 0, got ${progress.inProgress}`
    );
    assert(
      progress.pending === 2,
      `Expected pending 2, got ${progress.pending}`
    );
  } finally {
    env.cleanup();
  }
...
test('7. getMilestoneProgress handles project with zero tasks', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected completed 0, got ${progress.completed}`
    );
    assert(
      progress.inProgress === 0,
      `Expected inProgress 0, got ${progress.inProgress}`
    ...
    assert(
      progress.pending === 0,
      `Expected pending 0, got ${progress.pending}`
    );
    assert(
      progress.percentage === 0,
      `Expected percentage 0, got ${progress.percentage}`
    );
  } finally {
    env.cleanup();
  }
...
test('8. percentage is Math.round for non-integer percentages', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected percentage 33, got ${progress.percentage}`
    );
  } finally {
    env.cleanup();
  }
...
test('9. getAllMilestonesProgress returns all projects', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected 3 project entries, got ${allProgress.length}`
    );
  } finally {
    env.cleanup();
  }
  ...
  try {
    ...
    for (const entry of allProgress) {
      assert(
        ...
        `${entry.projectName} should have numeric total`
      );
      assert(
        typeof entry.completed === 'number',
        `${entry.projectName} should have numeric completed`
      );
      assert(
        typeof entry.inProgress === 'number',
        `${entry.projectName} should have numeric inProgress`
      );
      assert(
        typeof entry.pending === 'number',
        `${entry.projectName} should have numeric pending`
      );
      assert(
        typeof entry.percentage === 'number',
        `${entry.projectName} should have numeric percentage`
      );
    }
  } finally {
    env.cleanup();
...
test('12. getAllMilestonesProgress handles missing task files gracefully', () => {
  ...
  try {
    ...
    assert(
      ...
      `Expected 2 entries, got ${allProgress.length}`
    );
    const missing = allProgress.find(
      (p) => p.projectName === 'missing-project'
    );
    assert(missing != null, 'Should include missing-project');
    assert(
      missing.total === 0,
      `Missing project should have 0 tasks, got ${missing.total}`
    );
  } finally {
    env.cleanup();
  }
...
test('17. renderMilestoneTable handles empty progress array', () => {
  ...
  assert(
    ...
    'Output should be a string even for empty input'
  );

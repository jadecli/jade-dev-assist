...
function createMockExecGh(responses) {
    return function mockExecGh(args, options = {}) {
        ...
        if (options.json && typeof response.output === 'string' && response.output.trim()) {
            try {
...
function createTestEnv(tasks) {
    const tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'jade-github-sync-test-'));
    ...
    fs.writeFileSync(path.join(taskDir, 'tasks.json'), JSON.stringify(tasksData, null, 2));
...
test('1. formatIssueBody includes task ID in metadata section', () => {
    ...
    assert(body.includes('**Task ID:** `test-project/my-task`'), 'Should include task ID');
});
...
test('2. formatIssueBody includes description section', () => {
    ...
    assert(body.includes('This is a test task description.'), 'Should include description text');
});
...
test('3. formatIssueBody includes acceptance criteria as checkboxes', () => {
    ...
    assert(body.includes('## Acceptance Criteria'), 'Should have acceptance criteria section');
    assert(body.includes('- [ ] Criterion 1'), 'Should include first criterion as checkbox');
    assert(body.includes('- [ ] Criterion 2'), 'Should include second criterion as checkbox');
    assert(body.includes('- [ ] Criterion 3'), 'Should include third criterion as checkbox');
});
...
test('5. formatIssueBody includes relevant files section', () => {
    ...
    assert(body.includes('## Relevant Files'), 'Should have relevant files section');
...
test('6. formatIssueBody includes footer attribution', () => {
    ...
    assert(body.includes('Synced from local tasks.json by jade-dev-assist'), 'Should have footer');
});
...
- **Status:** pending`;
    ...
    assert(taskId === 'test-project/my-task', `Expected 'test-project/my-task', got '${taskId}'`);
});
...
test('9. extractTaskIdFromBody handles null/undefined body', () => {
    assert(extractTaskIdFromBody(null) === null, 'Should return null for null body');
    assert(extractTaskIdFromBody(undefined) === null, 'Should return null for undefined body');
    assert(extractTaskIdFromBody('') === null, 'Should return null for empty body');
});
...
test('11. createIssueFromTask builds correct labels from task status and complexity', () => {
    ...
    assert(statusLabel === 'status:in-progress', `Expected 'status:in-progress', got '${statusLabel}'`);
    assert(complexityLabel === 'size:large', `Expected 'size:large', got '${complexityLabel}'`);
});
...
- **Status:** pending`,
    ...
    assert(result.success === true, 'Should return success');
    assert(result.taskId === 'test-project/task-1', `Expected task ID, got '${result.taskId}'`);
    assert(result.newStatus === 'in_progress', `Expected 'in_progress', got '${result.newStatus}'`);
    assert(result.dryRun === true, 'Should indicate dryRun mode');
});
...
test('16. syncIssueToTask returns completed status for closed issues', () => {
    ...
    assert(result.success === true, 'Should return success');
    assert(result.newStatus === 'completed', 'Closed issues should map to completed status');
});
...
test('17. syncIssueToTask fails when task ID cannot be extracted', () => {
    ...
    assert(result.error.includes('Could not extract task ID'), 'Error should mention task ID');
});
...
test('18. syncIssueToTask succeeds with null status when no status label found', () => {
    ...
    assert(result.success === true, 'Should return success (no error, just no status)');
    assert(result.taskId === 'test-project/task-3', 'Should still extract task ID');
    assert(result.newStatus === null, 'newStatus should be null when no status label');
});
...
test('19. syncIssueToTask updates tasks.json when not in dryRun mode', () => {
    ...
    try {
        ...
        assert(updatedTask.status === 'in_progress', `Task status should be 'in_progress', got '${updatedTask.status}'`);
        assert(Array.isArray(updatedTask.history), 'Task should have history array');
        assert(updatedTask.history.length > 0, 'History should have at least one entry');
    } finally {
...
test('22. DEFAULT_LABEL_MAPPING has reverse status mapping', () => {
    ...
    assert(reverseStatus['status:pending'] === 'pending', 'Should reverse map status:pending');
    assert(reverseStatus['status:in-progress'] === 'in_progress', 'Should reverse map status:in-progress');
    assert(reverseStatus['status:completed'] === 'completed', 'Should reverse map status:completed');
});
...
test('23. syncTasksToIssues in dryRun mode processes tasks without errors', () => {
    ...
    try {
        const tasks = [
            { id: 'proj/task-1', title: 'Task 1', status: 'pending', complexity: 'S' },
            { id: 'proj/task-2', title: 'Task 2', status: 'in_progress', complexity: 'M', github_issue: 'owner/repo#123' }
        ];
...
test('24. syncTasksToIssues skips completed tasks without github_issue', () => {
    ...
    try {
        const tasks = [
            { id: 'proj/task-1', title: 'Completed Task', status: 'completed', complexity: 'M' }
        ];

        const result = syncTasksToIssues(tasks, { dryRun: true });

        // Completed task without github_issue should be skipped (no create attempt)
        assert(result.created === 0, 'Should not create issue for completed task without existing issue');
    } finally {
        process.stderr.write = originalWrite;
    }
});
...
test('25. Module exports all required functions', () => {
    assert(typeof githubSync.createIssueFromTask === 'function', 'Should export createIssueFromTask');
    assert(typeof githubSync.updateIssueFromTask === 'function', 'Should export updateIssueFromTask');
    assert(typeof githubSync.closeIssueOnComplete === 'function', 'Should export closeIssueOnComplete');
    assert(typeof githubSync.syncIssueToTask === 'function', 'Should export syncIssueToTask');
});

test('26. Module exports batch operations', () => {
    assert(typeof githubSync.syncTasksToIssues === 'function', 'Should export syncTasksToIssues');
    assert(typeof githubSync.syncIssuesToTasks === 'function', 'Should export syncIssuesToTasks');
});

test('27. Module exports utility functions', () => {
    assert(typeof githubSync.checkGhCli === 'function', 'Should export checkGhCli');
    assert(typeof githubSync.fetchOpenIssues === 'function', 'Should export fetchOpenIssues');
    assert(typeof githubSync.fetchIssue === 'function', 'Should export fetchIssue');
    assert(typeof githubSync.formatIssueBody === 'function', 'Should export formatIssueBody');
    assert(typeof githubSync.extractTaskIdFromBody === 'function', 'Should export extractTaskIdFromBody');
    assert(typeof githubSync.execGh === 'function', 'Should export execGh');
});

test('28. Module exports configuration constants', () => {
    assert(githubSync.DEFAULT_LABEL_MAPPING !== undefined, 'Should export DEFAULT_LABEL_MAPPING');
    assert(githubSync.DEFAULT_OPTIONS !== undefined, 'Should export DEFAULT_OPTIONS');
});

...
function createTestEnvironment(options) {
    const opts = options || {};
    const tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'jade-integration-test-'));
...
test('1. scanTasks loads tasks from all projects in registry', () => {
    ...
    try {
        ...
        const taskA = tasks.find(t => t.id === 'test-project-a/task-high-priority');
        assert(taskA != null, 'Should find task-high-priority');
        assert(taskA._project != null, 'Task should have _project metadata');
        assert(taskA._project.name === 'test-project-a', 'Task should have correct project name');
        assert(taskA._projectName === 'test-project-a', 'Task should have _projectName');
    } finally {
        env.cleanup();
    }
});
...
test('2. scanTasks handles projects with missing tasks.json gracefully', () => {
    ...
    const noTasksPath = path.join(env.tmpdir, 'no-tasks', '.claude', 'tasks', 'tasks.json');
...
test('4. scoreTasks filters out completed tasks', () => {
    ...
    try {
        ...
        assert(scored[0].id === 'test-project-a/pending-task',
...
test('5. scoreTasks considers dependency status in scoring', () => {
    ...
    try {
        ...
        const dependentTask = scored.find(t => t.id === 'test-project-a/task-depends');
        assert(dependentTask != null, 'Should find dependent task');

        // The blocked task should have lower score due to dependency factor
        const unblockedTask = scored.find(t => t.id === 'test-project-a/task-high-priority');
        assert(unblockedTask._score > dependentTask._score,
...
test('6. dispatchWorker builds prompt and updates status to in_progress', () => {
    ...
    try {
        ...
        assert(result.prompt.includes(project.name), 'Prompt should include project name');
        assert(result.prompt.includes(topTask.title), 'Prompt should include task title');
        assert(typeof result.workingDirectory === 'string', 'Should have working directory');
        assert(result.maxTurns === 25, `Expected maxTurns 25, got ${result.maxTurns}`);
...
test('7. dispatchWorker includes relevant files in prompt', () => {
    ...
    try {
        ...
        assert(result.prompt.includes('src/index.js'),
            'Prompt should include relevant file path');
        assert(result.meta.filesIncluded >= 1,
...
test('8. dispatchWorker adds history entry on status change', () => {
    ...
    try {
        ...
        const historyEntry = updatedTask.history.find(h => h.to_status === 'in_progress');
        assert(historyEntry != null, 'Should have in_progress history entry');
        assert(historyEntry.from_status === 'pending',
            `Expected from_status "pending", got "${historyEntry.from_status}"`);
        assert(typeof historyEntry.timestamp === 'string', 'History entry should have timestamp');
    } finally {
        env.cleanup();
    }
});
...
queueAsync('9. watchWorkerCompletion updates status to completed on exit 0', async () => {
    ...
    try {
        ...
        const tasksJsonPath = env.getTasksJsonPath('test-project-a');
        const updatedTask = readTaskFromFile(tasksJsonPath, topTask.id);
        assert(updatedTask.status === 'completed',
...
queueAsync('10. successful completion adds history entry', async () => {
    ...
    try {
        ...
        const completedEntry = updatedTask.history.find(h => h.to_status === 'completed');
        assert(completedEntry != null, 'Should have completed history entry');
        assert(completedEntry.from_status === 'in_progress',
...
queueAsync('11. completion result includes stdout and completedAt', async () => {
    ...
    try {
        ...
        assert(result.stdout.includes('completed successfully'),
            'stdout should contain success message');
        assert(typeof result.completedAt === 'string', 'Result should have completedAt');
        assert(!isNaN(Date.parse(result.completedAt)), 'completedAt should be valid ISO date');
    } finally {
        env.cleanup();
    }
});
...
queueAsync('12. watchWorkerCompletion updates status to failed on non-zero exit', async () => {
    ...
    try {
        ...
        assert(result.exitCode === 1, `Expected exitCode 1, got ${result.exitCode}`);

        const tasksJsonPath = env.getTasksJsonPath('test-project-a');
        const updatedTask = readTaskFromFile(tasksJsonPath, topTask.id);
        assert(updatedTask.status === 'failed',
...
queueAsync('13. failed completion adds history entry with error details', async () => {
    ...
    try {
        ...
        const failedEntry = updatedTask.history.find(h => h.to_status === 'failed');
        assert(failedEntry != null, 'Should have failed history entry');
        assert(typeof failedEntry.agent_summary === 'string',
            'Failed entry should have agent_summary with error details');
        assert(failedEntry.agent_summary.includes('exit'),
...
queueAsync('15. full cycle: scan -> score -> dispatch -> complete', async () => {
    const env = createTestEnvironment();
    try {
        // Phase 1: Scan
        const tasks = scanTasks({ registryPath: env.projectsJsonPath }).tasks;
        ...
        assert(dispatchResult.prompt.length > 100,
        ...
        assert(currentTask.status === 'in_progress',
            'Task should be in_progress after dispatch');

        // Phase 4: Complete (success)
        const mockChild = createMockWorkerProcess(0);
        const completionResult = await watchWorkerCompletion(topTask.id, mockChild, {
            projectsRoot: env.tmpdir
        });

        assert(completionResult.exitCode === 0, 'Worker should exit successfully');

        // Verify final state
        currentTask = readTaskFromFile(tasksJsonPath, topTask.id);
        assert(currentTask.status === 'completed',
            `Task should be completed, got "${currentTask.status}"`);

        // Verify history has both transitions
        assert(currentTask.history.length >= 2,
            `Expected at least 2 history entries, got ${currentTask.history.length}`);
        const inProgressEntry = currentTask.history.find(h => h.to_status === 'in_progress');
        const completedEntry = currentTask.history.find(h => h.to_status === 'completed');
...
queueAsync('16. full cycle: scan -> score -> dispatch -> fail', async () => {
    ...
    try {
        ...
        assert(currentTask.status === 'failed',
...
test('18. handles empty task list gracefully', () => {
    ...
    try {
        ...
        const scored = scoreTasks(tasks);
        assert(scored.length === 0, `Expected 0 scored tasks, got ${scored.length}`);
    } finally {
        env.cleanup();
    }
});
...
queueAsync('20. handles delayed worker completion', async () => {
    ...
    try {
        ...
        assert(result.exitCode === 0, 'Delayed worker should complete successfully');

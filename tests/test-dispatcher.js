...
function createTestProject(options) {
    const opts = options || {};
    const tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'jade-dispatcher-test-'));
...
test('1. TOKEN_BUDGET is 60000', () => {
    ...
    assert(TOKEN_BUDGET === 60000, `TOKEN_BUDGET should be 60000, got ${TOKEN_BUDGET}`);
});
...
test('5. buildWorkerPrompt returns an object with prompt and meta', () => {
    ...
    try {
        ...
        assert(typeof result.prompt === 'string', 'result.prompt should be a string');
...
test('7. buildWorkerPrompt prompt includes task description', () => {
    ...
    try {
        ...
        assert(
            result.prompt.includes('Add a frobnicator that processes widgets efficiently.'),
...
test('23. prompt includes Your Assignment section', () => {
    ...
    try {
        ...
        assert(
            result.prompt.includes('Your Assignment') || result.prompt.includes('Assignment'),
...
test('26. prompt includes TDD skill reference', () => {
    ...
    try {
        ...
        assert(
            result.prompt.includes('test-driven-development') || result.prompt.includes('TDD'),
...
test('27. dispatchWorker returns a dispatch descriptor', () => {
    ...
    try {
        ...
        assert(result != null, 'Result should not be null');
        assert(typeof result.prompt === 'string', 'result.prompt should be a string');
        assert(typeof result.workingDirectory === 'string', 'result.workingDirectory should be a string');
        assert(typeof result.maxTurns === 'number', 'result.maxTurns should be a number');
    } finally {
        env.cleanup();
    }
});
...
test('30. dispatchWorker updates task status to in_progress in tasks.json', () => {
    ...
    try {
        ...
        const updatedTask = updatedData.tasks.find(t => t.id === 'test-project/test-task');
...
test('37. meta.filesTrimmed counts files excluded due to budget', () => {
    ...
    try {
        ...
        assert(
            result.meta.filesTrimmed >= 0,
            'meta.filesTrimmed should be >= 0'
        );
    } finally {
        env.cleanup();
    }

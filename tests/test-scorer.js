...
function makeProject(overrides) {
    return Object.assign({ name: 'test-project', status: 'scaffolding' }, overrides);
}
...
test('1. MATURITY_SCORES maps all 5 statuses', () => {
    ...
    assert(MATURITY_SCORES.buildable === 100, `buildable should be 100, got ${MATURITY_SCORES.buildable}`);
    assert(MATURITY_SCORES['near-buildable'] === 80, `near-buildable should be 80, got ${MATURITY_SCORES['near-buildable']}`);
    assert(MATURITY_SCORES['scaffolding-plus'] === 60, `scaffolding-plus should be 60, got ${MATURITY_SCORES['scaffolding-plus']}`);
    assert(MATURITY_SCORES.scaffolding === 40, `scaffolding should be 40, got ${MATURITY_SCORES.scaffolding}`);
    assert(MATURITY_SCORES.blocked === 10, `blocked should be 10, got ${MATURITY_SCORES.blocked}`);
});

test('2. COMPLEXITY_MULTIPLIERS maps S/M/L/XL', () => {
    assert(COMPLEXITY_MULTIPLIERS != null, 'COMPLEXITY_MULTIPLIERS should be defined');
    assert(COMPLEXITY_MULTIPLIERS.S === 1.0, `S should be 1.0, got ${COMPLEXITY_MULTIPLIERS.S}`);
    assert(COMPLEXITY_MULTIPLIERS.M === 0.75, `M should be 0.75, got ${COMPLEXITY_MULTIPLIERS.M}`);
    assert(COMPLEXITY_MULTIPLIERS.L === 0.50, `L should be 0.50, got ${COMPLEXITY_MULTIPLIERS.L}`);
    assert(COMPLEXITY_MULTIPLIERS.XL === 0.30, `XL should be 0.30, got ${COMPLEXITY_MULTIPLIERS.XL}`);
});

test('3. WEIGHTS has correct values summing to 1.0', () => {
    assert(WEIGHTS != null, 'WEIGHTS should be defined');
    assert(WEIGHTS.maturity === 0.20, `maturity weight should be 0.20, got ${WEIGHTS.maturity}`);
    assert(WEIGHTS.impact === 0.30, `impact weight should be 0.30, got ${WEIGHTS.impact}`);
    assert(WEIGHTS.dependency === 0.20, `dependency weight should be 0.20, got ${WEIGHTS.dependency}`);
    assert(WEIGHTS.effort === 0.15, `effort weight should be 0.15, got ${WEIGHTS.effort}`);
    assert(WEIGHTS.preference === 0.15, `preference weight should be 0.15, got ${WEIGHTS.preference}`);
    const sum = WEIGHTS.maturity + WEIGHTS.impact + WEIGHTS.dependency + WEIGHTS.effort + WEIGHTS.preference;
...
test('17. dependency: all blockers in_progress = 50', () => {
    ...
    // = 8 + 0 + 10 + 0 + 7.5 = 25.5
    const score = scoreTask(task, project, allTasks);
    assertClose(score, 25.5, 0.01, 'all blockers in_progress gives dependency=50');
});
...
test('18. dependency: mix of completed and in_progress = 50', () => {
    ...
    // = 8 + 0 + 10 + 0 + 7.5 = 25.5
    const score = scoreTask(task, project, allTasks);
    assertClose(score, 25.5, 0.01, 'completed+in_progress mix gives dependency=50');
});
...
test('35. Example 1: jade-cli/fix-node-build = 78.0', () => {
    ...
    const task = makeTask({
        ...
        feature: {
            ...
            acceptance_criteria: ['npm run build exits 0', 'tests pass', 'built output works']
        },
...
test('36. Example 2: jade-ide/research-vscode-api = 12.95', () => {
    ...
    const task = makeTask({
        ...
        unlocks: [],
        labels: [],
        feature: { description: 'Understanding VS Code extension API capabilities' },
    ...
    assertClose(score, 12.95, 0.01, 'jade-ide/research-vscode-api should score 12.95');
});
...
test('37. scoreTasks returns sorted array with _score property', () => {
    ...
    assert(result[0]._score >= result[1]._score, 'Should be sorted descending by score');
...
test('38. scoreTasks filters out completed tasks by default', () => {
    ...
    assert(result.length === 1, `Expected 1 task after filtering, got ${result.length}`);
...
test('40. scoreTasks includeCompleted option keeps completed/failed tasks', () => {
    ...
    const result = scoreTasks([pending, completed, failedTask], { includeCompleted: true });
    assert(result.length === 3, `Expected 3 tasks with includeCompleted, got ${result.length}`);
});
...
test('41. scoreTasks passes focusLabel option through to scoreTask', () => {
    ...
    assert(result[0].id === 'proj/focused',
...
test('43. scoreTasks uses task._project for project lookup', () => {
    ...
    // = 20 + 0 + 20 + 0 + 7.5 = 47.5
    assertClose(result[0]._score, 47.5, 0.01, 'Should use _project.status for maturity');
});
...
test('49. score is always between 0 and 100', () => {
    ...
    const maxScore = scoreTask(maxTask, maxProject, [maxTask], { focusLabel: 'bugfix' });

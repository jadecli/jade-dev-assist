...
test('.claude-plugin directory exists', () => {
    assert(fs.existsSync(path.join(PLUGIN_ROOT, '.claude-plugin')), 
        '.claude-plugin directory not found');
});

test('commands directory exists', () => {
    assert(fs.existsSync(path.join(PLUGIN_ROOT, 'commands')), 
        'commands directory not found');
});

test('skills directory exists', () => {
    assert(fs.existsSync(path.join(PLUGIN_ROOT, 'skills')), 
        'skills directory not found');
});

test('docs directory exists', () => {
    assert(fs.existsSync(path.join(PLUGIN_ROOT, 'docs')), 
...
test('plugin.json name is kebab-case', () => {
    const manifestPath = path.join(PLUGIN_ROOT, '.claude-plugin/plugin.json');
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    assert(/^[a-z][a-z0-9-]*[a-z0-9]$/.test(manifest.name), 
...
test('personalization-manager skill exists', () => {
    const skillPath = path.join(PLUGIN_ROOT, 'skills/personalization-manager/SKILL.md');
    assert(fs.existsSync(skillPath), 'personalization-manager SKILL.md not found');
});

test('style-configurator skill exists', () => {
    const skillPath = path.join(PLUGIN_ROOT, 'skills/style-configurator/SKILL.md');
    assert(fs.existsSync(skillPath), 'style-configurator SKILL.md not found');
});

test('capability-toggler skill exists', () => {
    const skillPath = path.join(PLUGIN_ROOT, 'skills/capability-toggler/SKILL.md');
...
test('README.md exists', () => {
    assert(fs.existsSync(path.join(PLUGIN_ROOT, 'README.md')), 
        'README.md not found');
});

test('LICENSE exists', () => {
    assert(fs.existsSync(path.join(PLUGIN_ROOT, 'LICENSE')), 
        'LICENSE not found');
});
...
test('Research documentation exists', () => {
    ...
    assert(fs.existsSync(summaryPath), 'claude-documentation-summary.md not found');
});

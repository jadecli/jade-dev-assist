...
function createTestEnv() {
    const tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'jade-issue-mapper-test-'));
...
test('1. getMapPath returns correct path', () => {
    ...
    assert(mapPath.endsWith('issue-map.json'), 'Path should end with issue-map.json');
});
...
test('2. loadMap returns empty maps for non-existent file', () => {
    ...
    try {
        ...
        assert(Object.keys(map.taskToIssue).length === 0, 'taskToIssue should be empty');
        assert(Object.keys(map.issueToTask).length === 0, 'issueToTask should be empty');
    } finally {
        env.cleanup();
    }
});
...
test('3. saveMap creates map file', () => {
    ...
    try {
        ...
        assert(savedData.taskToIssue['proj/task-1'] === 42, 'Should have correct task mapping');
        assert(savedData.issueToTask['42'] === 'proj/task-1', 'Should have correct issue mapping');
    } finally {
        env.cleanup();
    }
});
...
test('4. loadMap reads saved map correctly', () => {
    ...
    try {
        ...
        assert(loadedMap.taskToIssue['proj/task-1'] === 100, 'Should load task-1 mapping');
        assert(loadedMap.taskToIssue['proj/task-2'] === 101, 'Should load task-2 mapping');
        assert(loadedMap.issueToTask['100'] === 'proj/task-1', 'Should load issue 100 mapping');
        assert(loadedMap.issueToTask['101'] === 'proj/task-2', 'Should load issue 101 mapping');
    } finally {
        env.cleanup();
    }
});
...
test('6. registerMapping persists to disk', () => {
    ...
    try {
        ...
        const savedData = env.readMap();
        assert(savedData.taskToIssue['proj/task-2'] === 99, 'Should persist task mapping');
        assert(savedData.issueToTask['99'] === 'proj/task-2', 'Should persist issue mapping');
    } finally {
        process.stderr.write = originalWrite;
        env.cleanup();
    }
...
test('12. getAllMappings returns all registered mappings', () => {
    ...
    try {
        ...
        assert(Object.keys(all.taskToIssue).length === 3, 'Should have 3 task mappings');
        assert(Object.keys(all.issueToTask).length === 3, 'Should have 3 issue mappings');
    } finally {
        process.stderr.write = originalWrite;
        env.cleanup();
    }
...
test('13. clearMappings removes all mappings', () => {
    ...
    try {
        ...
        const all = getAllMappings(env.projectRoot);
        assert(Object.keys(all.taskToIssue).length === 0, 'taskToIssue should be empty');
        assert(Object.keys(all.issueToTask).length === 0, 'issueToTask should be empty');
    } finally {
        process.stderr.write = originalWrite;
        env.cleanup();
    }
...
test('14. Module exports all required functions', () => {
    assert(typeof issueMapper.getIssueForTask === 'function', 'Should export getIssueForTask');
    assert(typeof issueMapper.getTaskForIssue === 'function', 'Should export getTaskForIssue');
    assert(typeof issueMapper.registerMapping === 'function', 'Should export registerMapping');
    assert(typeof issueMapper.removeTaskMapping === 'function', 'Should export removeTaskMapping');
    assert(typeof issueMapper.removeIssueMapping === 'function', 'Should export removeIssueMapping');
    assert(typeof issueMapper.getAllMappings === 'function', 'Should export getAllMappings');
    assert(typeof issueMapper.clearMappings === 'function', 'Should export clearMappings');
    assert(typeof issueMapper.syncMappingsToTasksJson === 'function', 'Should export syncMappingsToTasksJson');
});

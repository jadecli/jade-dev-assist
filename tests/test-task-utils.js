...
function createTestProject(options) {
  ...
  const tmpdir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'jade-task-utils-test-')
  );
...
test('parseTaskId: throws on missing slash', () => {
  assertThrows(() => parseTaskId('invalid-task-id'), 'Invalid task ID format');
});

test('parseTaskId: throws on empty string', () => {
  assertThrows(() => parseTaskId(''), 'Invalid task ID format');
});
...
test('getTasksJsonPath: builds correct path', () => {
  ...
  assertEquals(result, '/home/user/projects/jade-cli/.claude/tasks/tasks.json');
});
...
test('findTask: returns correct taskIndex for first task', () => {
  const testProj = createTestProject({
    ...
    tasksData: {
      ...
      tasks: [
        {
          id: 'test-project/first',
          title: 'First',
          status: 'pending',
          complexity: 'S',
        },
        {
          id: 'test-project/second',
          title: 'Second',
          status: 'pending',
          complexity: 'S',
        },

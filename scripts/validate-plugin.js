...
const REQUIRED_DIRS = ['.claude-plugin', 'commands', 'skills'];

const REQUIRED_FILES = ['.claude-plugin/plugin.json', 'README.md', 'LICENSE'];
  ...
  validateManifest() {
    ...
    try {
      ...
      if (manifest.name && !/^[a-z][a-z0-9-]*[a-z0-9]$/.test(manifest.name)) {
        this.warnings.push(
          `Plugin name should be kebab-case: ${manifest.name}`
        );
      }
      ...
      if (manifest.commands) {
        for (const cmd of manifest.commands) {
          if (!cmd.name || !cmd.path) {
            this.errors.push(
              `Command missing name or path: ${JSON.stringify(cmd)}`
            );
          }
      ...
      if (manifest.skills) {
        for (const skill of manifest.skills) {
          if (!skill.name || !skill.path) {
            this.errors.push(
              `Skill missing name or path: ${JSON.stringify(skill)}`
            );
          }
    ...
    for (const skill of skills) {
      ...
      if (frontmatterMatch) {
        ...
        if (!frontmatter.includes('description:')) {
          this.errors.push(
            `Skill ${skill} frontmatter missing 'description' (required for skill discovery)`
          );
        }
      }
    }

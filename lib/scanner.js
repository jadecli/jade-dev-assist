...
const KNOWN_TASK_FIELDS = new Set([
    ...
    'complexity', 'blocked_by', 'unlocks',
    // Other known fields
    'description', 'milestone', 'labels', 'feature',
    'github_issue', 'relevant_files', 'created_at',
    // Internal fields added by scanner
    '_project', '_projectName', '_milestone'
]);
...
function validateTask(task, filePath, options) {
    ...
    for (const field of Object.keys(task)) {
        if (!KNOWN_TASK_FIELDS.has(field)) {
            warnings.push(`Unknown field '${field}' in task '${task.id || 'unknown'}' at ${filePath}`);
        }
    }
...
function applyTaskDefaults(task) {
    for (const [field, defaultValue] of Object.entries(OPTIONAL_TASK_FIELDS)) {
        if (task[field] === undefined) {
            task[field] = Array.isArray(defaultValue) ? [...defaultValue] : defaultValue;
        }
    }
    return task;
}

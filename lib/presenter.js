...
function presentTasks(rankedTasks, options) {
    ...
    // Top border
    lines.push(horizontalRule(BOX.topLeft, BOX.topTee, BOX.topRight, COLUMN_WIDTHS));

    // Column headers
    lines.push(dataRow([' #', 'Project', 'Task', 'Score', 'Size'], COLUMN_WIDTHS, ['right', 'left', 'left', 'right', 'left']));

    // Separator
    lines.push(horizontalRule(BOX.leftTee, BOX.cross, BOX.rightTee, COLUMN_WIDTHS));
    ...
    for (let i = 0; i < tasksToShow.length; i++) {
        ...
        lines.push(dataRow([rank, project, title, score, size], COLUMN_WIDTHS, ['right', 'left', 'left', 'right', 'left']));
    }

    // Bottom border
    lines.push(horizontalRule(BOX.bottomLeft, BOX.bottomTee, BOX.bottomRight, COLUMN_WIDTHS));
    lines.push('');

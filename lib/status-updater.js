...
/**
 * Resolve the tasks.json path and find a task by ID.
 *
 ...
 * @returns {{ task: Object, taskIndex: number, data: Object, tasksJsonPath: string }}
...
function findTask(taskId, projectsRoot) {
    ...
    return { task: tasks[taskIndex], taskIndex, data, tasksJsonPath };
}
        ...
        childProcess.on('close', (exitCode) => {
            ...
            if (exitCode === 0) {
                ...
                try {
                    ...
                    console.warn(`Warning: could not update task status to completed: ${err.message}`);
                }
            } else {
                // Failure -- update to failed with error details
                const errorSummary = `Worker exited with code ${exitCode}` +
                ...
                try {
                    ...
                    console.warn(`Warning: could not update task status to failed: ${err.message}`);
                }
            }

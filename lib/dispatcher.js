 ...
 * 2. Updates the task status to in_progress in tasks.json.
 * 3. Returns a dispatch descriptor with the prompt, working directory,
...
function dispatchWorker(task, project, options) {
    ...
    try {
        ...
        if (err.code !== 'ENOENT') {
            console.warn(
                `Warning: could not update task status in ${tasksJsonPath}: ${err.message}`
            );
        }
    }

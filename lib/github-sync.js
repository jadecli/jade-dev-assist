  ...
  try {
    ...
    if (result.status !== 0 && !options.ignoreErrors) {
      const error = new Error(
        `gh command failed: ${result.stderr || 'Unknown error'}`
      );
...
function formatIssueBody(task) {
  ...
  if (task.feature) {
    ...
    if (
      task.feature.acceptance_criteria &&
      task.feature.acceptance_criteria.length > 0
    ) {
    ...
    if (task.github_issue) {
      ...
      if (match) {
        ...
        if (updateResult.success) {
          ...
          results.errors.push(
            `Failed to update issue #${issueNumber}: ${updateResult.error}`
          );
        }
      }
    } else {
      ...
      if (createResult.success) {
        ...
        results.errors.push(
          `Failed to create issue for ${task.id}: ${createResult.error}`
        );
      }
    }
  }
...
function syncIssuesToTasks(options = {}) {
  ...
  for (const issue of fetchResult.issues) {
    ...
    if (syncResult.success && syncResult.newStatus) {
      ...
      results.errors.push(
        `Failed to sync issue #${issue.number}: ${syncResult.error}`
      );
    }
  }
